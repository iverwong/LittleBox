...
async def test_e2e_enqueue_to_ready(concurrent_db_sessions):
    ...
    with (
        patch("app.auth.redis_client.get_audit_redis", return_value=shared_redis),
        patch("arq.create_pool", return_value=mock_arq),
    ):
        await enqueue_audit(sid, db, turn_number=1)
    ...
    with patch("app.audit.worker.build_audit_graph", return_value=fake_graph):
        await run_audit({"redis": shared_redis, "job_try": 1}, str(sid), 1)
