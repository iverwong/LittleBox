...
契约:
- get_client_ip(request) 只读 request.client.host, 不解析任何代理头
...
def _make_request(
    *,
    ...
    xff / xri 参数仅用于回归测试, 验证 get_client_ip 即便收到伪造代理头
...
class TestGetClientIP:
    """get_client_ip(request) 只读 peer IP, 不解析任何代理头。"""
...
class TestGetClientIPIgnoresProxyHeaders:
    ...
    def test_attacker_rotating_xff_does_not_bypass(self) -> None:
        """XFF 轮换攻击回归: 即便攻击者每请求换一个 XFF[0], get_client_ip
        ...
        for i in range(25):
            ...
            assert get_client_ip(req) == "attacker.1.2.3.4"
