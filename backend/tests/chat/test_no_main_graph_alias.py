...
验证 T8 工厂化后 `app.domain.chat.graph.main_graph` 已删除（无向后兼容别名）。
"""
from __future__ import annotations


def test_no_main_graph_alias():
    """app.domain.chat.graph 不存在 main_graph 模块级别名（T8 工厂化时已删）。"""
    import importlib

    mod = importlib.import_module("app.domain.chat.graph")
