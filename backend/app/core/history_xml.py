"""LangChain message 与 XML 字符串的相互转换工具。

上下文压缩、危机 / 红线干预 LLM 调用、audit history XML 包装都共用本模块。
将多轮 history 包装为 XML 字符串而非真实 `HumanMessage` / `AIMessage` 序列，
避免 chat template 把待压缩的 assistant 末帧误认为 generation prefix 触发续写。

设计约束：
- 序列化输出禁止使用 `Human:` / `AI:` / `[AI]` 等与 chat template 角色标记
  同形的文本前缀；
- content 必须 XML 转义；
- 配套 `extract_wrapped_output` 用于从 LLM 输出中按 tag 提取内容。
"""
...
def escape_xml_text(text: str) -> str:
    ...
    仅处理文本节点，不处理属性引号；调用方勿把用户输入塞进属性值。
...
def extract_wrapped_output(raw: str, tag: str) -> str | None:
    ...
    容忍前后空白与 markdown code fence；多次出现取第一段；找不到返回 `None`
    （调用方决定兜底策略）。`tag` 必须是简单标识符（字母 / 下划线）；
    本函数不做 tag 输入校验，调用方传字面常量即可。

    Args:
        raw: LLM 原始输出字符串。
        tag: 目标 tag 名（如 `"crisis_turn"`）。

    Returns:
        tag 内部内容（去首尾空白），或 `None`。
    """
