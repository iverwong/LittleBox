...
async def load_active_messages_with_summary(
    ...
    """取指定会话当前 active 的消息列表与当前 active summary(0 或 1 条)。

    summary 是上下文压缩的产物:每次压缩在 messages 表插入一条 role=summary、
    status=active 的行,旧的 active summary 在新一轮压缩时被标 compressed,因此任何时刻
    active summary 至多 1 条。该消息专供 build_system_prompt 注入,不进 history 列表
    (避免双写)。

    Args:
        sid: session UUID。
        db: 异步 DB session。
        from_turn: 可选起始轮号(闭区间)。
        to_turn: 可选终止轮号(闭区间)。

    Returns:
        (messages, summary):messages 为 active 的 human/ai 消息列表,summary 为当前
        active summary(无则 None)。
    """
...
async def load_recent_messages(
    ...
    返回指定 turn_number 闭区间内的所有 human/ai 消息(无 LIMIT 截断),
    ...
    Args:
        ...
        from_turn: 起始轮号(闭区间)。
        to_turn: 终止轮号(闭区间)。
        as_orm: True 返回 ORM Message 列表,False 返回 LangChain BaseMessage 列表。

    Returns:
        按 created_at 升序排列的消息序列。
    """
    rows = (
        (
            await db.execute(
...
def to_lc_message(m: Message) -> BaseMessage:
    ...
    Returns:
        对应的 LangChain 消息(human → HumanMessage, ai → AIMessage,
        summary → 带前缀的 SystemMessage,未知 role 兜底 HumanMessage)。
    """
