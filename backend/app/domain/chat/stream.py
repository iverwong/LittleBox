...
async def stream_graph_to_sse(payloads) -> AsyncIterator[bytes]:
    ...
    Note: 本函数不追踪跨 payload 的 thinking_started 状态（单个 payload
    传给 stream_graph_to_sse，无法知道前后 payload 是否有 reasoning 信号）。
    pipeline.py 的 graph 循环层在入 payload 前已处理 thinking_started/
    thinking_end 的 SSE 事件发射（_put），本函数只需将 delta 映射为 SSE 帧。
    """
