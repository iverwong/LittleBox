...
class AuditWaitResult:
    ...
    Attributes:
        kind: 5 种终端状态之一。
        signals: status=ready 时携带的审查结果;其余场景为 None。
        actual_turn: 实际管道中的 turn(turn_mismatch 时填充)。
...
class AuditSignalsManager:
    ...
    def __init__(
        ...
        """构造 AuditSignalsManager。

        Args:
            redis: 审查信号管道所在的 Redis 客户端(通常是 audit_redis,
                即 db=arq_redis_db 的实例)。
            ttl: SET 时写入的过期秒数,通常等于 config.audit_redis_ttl_seconds。
            poll_interval: poll_wait 两次轮询之间的 sleep 秒数。
            poll_timeout: poll_wait 默认超时秒数(测试可覆盖)。
        """
        self._redis = redis
        self._ttl = ttl
    ...
    async def set_pending(
        ...
        由 `enqueue_audit` 调用。started_at 由调用方传入(UTC ISO-8601)。

        Args:
            sid: 被审查的对话 session ID(字符串形式)。
            turn: 对应的 ai_turn 轮次。
            started_at: 入队起始时间(UTC ISO-8601 字符串)。
        """
    ...
    async def set_ready(
        ...
        Args:
            ...
            turn: 对应的 ai_turn 轮次。
            signals: 审查结果。
            completed_at: 完成时间(UTC ISO-8601 字符串),None 仅用于测试。
        """
        payload = AuditSignalsPayload(
            status="ready",
            turn=turn,
    ...
    async def set_failed(
        ...
        由 ARQ `run_audit` 在 `job_try >= MAX_TRIES` 时调用(arq 0.28 无
        `on_job_failure` 钩子)。completed_at 通常应由调用方传入;None 仅用于测试。
    ...
    async def get(self, sid: str) -> AuditSignalsPayload | None:
        ...
        Returns:
            解析成功返回 AuditSignalsPayload;key 不存在或 JSON 损坏 /
            schema 不匹配时返回 None(与 miss 等价降级)。
        """
    ...
    async def poll_wait(
        ...
        """轮询 `audit:{sid}` 直到终端状态或超时。

        6 路径分发(turn 严格不等即 mismatch,不论大小):
        - key 不存在 → miss
        ...
        Args:
            sid: 被审查的对话 session ID(字符串形式)。
            expected_turn: 调用方期望匹配的轮次。
            timeout: 自定义超时秒数;None 时取 self._poll_timeout
                (构造注入,prod=30s,测试可覆盖)。

        Returns:
            AuditWaitResult,5 种终端状态之一。
        """
