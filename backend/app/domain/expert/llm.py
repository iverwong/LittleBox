...
- 主端:`build_role_primary(Role.EXPERT, settings, http_async_client=...)` →
  deepseek + thinking + reasoning_effort=MAX(裸 `BaseChatModel`)
- 备端:`build_role_fallback(Role.EXPERT, settings, http_async_client=...)` →
  bailian + thinking + reasoning_effort=MAX(裸 `BaseChatModel`)
- 主备各 `bind_tools([SearchHistoryInput, FetchByRefInput, ExpertReportSchema])`,
  再 `wrap_resilience(..., retry_attempts=ROLES[Role.EXPERT].retry_attempts)`
  主端重试 + 备端兜底一气呵成
...
- `retry_attempts` 显式读 `ROLES[Role.EXPERT].retry_attempts`(今日 = 3)单一真相源,
...
from app.domain.expert.schemas import ExpertReportSchema, FetchByRefInput, SearchHistoryInput
...
def build_expert_llm(
    ...
    *,
    ...
    装配链:
        ...
        1. `build_role_primary(Role.EXPERT, settings, http_async_client=...)` →
           主端裸 ChatDeepSeek
        2. `build_role_fallback(Role.EXPERT, settings, http_async_client=...)` →
           备端裸 ChatDeepSeek(bailian)
        3. 主备各 `bind_tools([SearchHistoryInput, FetchByRefInput, ExpertReportSchema])`
        4. `wrap_resilience(primary_bound, fallback_bound, retry_attempts=
           ROLES[Role.EXPERT].retry_attempts)` → `with_retry(stop=3) + with_fallbacks`

    Retry 策略:`stop_after_attempt=ROLES[Role.EXPERT].retry_attempts`(今日 = 3),
    ...
    Raises:
        RuntimeError: 防御性 — 若 `ROLES[Role.EXPERT].fallback` 被配成 None,
    ...
    if fallback is None:
        raise RuntimeError(
            "expert role 必须有 fallback 配置(百炼兜底);"
            "若有意移除请同步改 app/domain/expert/llm.py::build_expert_llm"
        )
    ...
    return wrap_resilience(
        primary_bound,
        fallback_bound,
        retry_attempts=ROLES[Role.EXPERT].retry_attempts,
    )
