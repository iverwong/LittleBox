...
function renderCenter(
  ...
  if (typeof centerLabel === "string")
    return <Text style={styles.centerValue}>{centerLabel}</Text>;
  if (centerLabel != null) return centerLabel;
  return <Text style={styles.centerValue}>{String(value)}</Text>;
}

const THUMB_RADIUS = 12; // thumbOuter 外层 24×24 容器半径，不是内层视觉圆 18/2
...
export function DiscreteSlider({
  ...
  const activeIndex = useMemo(() => {
    ...
    if (idx < 0 && __DEV__) {
      console.warn(
        `[DiscreteSlider] value=${value} 不在 nodes 内，fallback 到最近节点`,
      );
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <GestureDetector gesture={pan}>
        <View
          style={[styles.trackRow, disabled && styles.trackRowDisabled]}
          onLayout={onTrackLayout}
        >
          {/* 轨道背景 */}
          <View style={[styles.trackBg, disabled && styles.trackBgDisabled]} />
          {/* 激活段 */}
          <Animated.View
            style={[
              styles.activeTrack,
