...
3. **`sessions.context_token_count INT` 累加器**：commit① / commit② 同事务内 `+= estimate_tokens(content)`；末行决策矩阵 discarded 路径 `-=`（§4.7）
4. **commit② 后内联阈值判定**：`if session.context_token_count >= 500_000: log.warning(...)`，O(1) 字段比较，**不 fire-and-forget**
5. Session 日切策略（4h 硬切兜底 + 凌晨 [1,4) 空闲 30min 切）+ 单 session 写入约束 + `last_active_at = user_msg.created_at` 纪律
6. 一次清理 `dev_chat` / `stream_chat` / `sse.py` dev 路径残留
7. `list_sessions` 响应顶层附 `today_session_id: UUID | null` + `sessions` 数组过滤今日 logical_day
8. `today_session_title` 返回 `周一 · 5月11日` 中文格式（数组取星期，不依赖 locale）
...
| J | 阈值触发副作用 | 仅 `log.warning(...)`，不写库；不建独立事件表；不污染 `rolling_summaries` |
| K | token 累加策略 | 新增 `sessions.context_token_count INT NOT NULL DEFAULT 0` 累加器。commit① / commit② 同事务 `+=`；discarded 路径同事务 `-=`。撤销 v1.0 草案的 `compress_context_if_needed` 函数；判定改为 commit② 之后内联 `if session.context_token_count >= THRESHOLD: log.warning(...)` |
...
- [ ]  `git checkout -b feat/m6-patch3-context-threshold-session-daily`
- [ ]  `pytest` 跑一遍确认 414 passed 基线
- [ ]  复读 §2 决策表 + 基线 §3.5 / §4.4-4.7 / §5.6 / §7.2 / §7.6
...
- [ ]  `backend/app/main.py` 删除 `sse_router` 导入与 `include_router` 注册
- [ ]  `rg 'dev_chat|stream_chat' backend/` 全仓清理（含 tests/ 与 inline docstring）
- [ ]  删除 `backend/tests/test_dev_chat.py`（若存在）
...
- [ ]  `chat/prompts.py` 文件头加纪律注释（中文）
- [ ]  把 `graph.py` / `me.py` / `context.py` 中所有 inline prompt 字面量迁入
- [ ]  新增 `COMPRESSION_PROMPT_STUB` + `build_compression_prompt() -> SystemMessage`（M8 用）
...
- [ ]  `alembic revision -m "m6_patch3_session_context_token_count"` 生成新 revision
- [ ]  revision 加字段 `sessions.context_token_count INTEGER NOT NULL DEFAULT 0`
- [ ]  新建 `backend/app/chat/compression.py`：仅 `estimate_tokens(content: str) -> int` 单函数 + `CONTEXT_COMPRESS_THRESHOLD_TOKENS = 500_000` 常量
- [ ]  更新 `backend/app/models/session.py` ORM 加字段
...
- [ ]  新建 `backend/app/chat/session_policy.py`
- [ ]  三常量 + 占位常量 + `logical_day` + `should_switch_session` + `today_session_title`（中文格式）
- [ ]  纯函数零副作用，便于单测
...
- [ ]  删 `LIMIT 20`
- [ ]  SELECT 全部 `status='active'` 消息按 `created_at` ASC
- [ ]  签名简化为 `async def build_context(sid, db) -> list[BaseMessage]`，**不**返回 token / 阈值
- [ ]  token 估算与阈值常量**不**留在本文件（仍在 `compression.py`）
...
- [ ]  Row 1 分支前查 `latest_session`（`child_user_id=current.id` 且 `status='active'`，`ORDER BY last_active_at DESC LIMIT 1`）
- [ ]  `now = datetime.now(SHANGHAI)`；调 `should_switch_session(latest.last_active_at if latest else None, now)`
- [ ]  True → 新建 session（标题 `today_session_title(now)`，`last_active_at=now`，`context_token_count=0`）；False → 复用 `latest.id`
- [ ]  commit① 同事务内：
    - `session.last_active_at = user_msg.created_at`（F 决策）
    - `session.context_token_count += estimate_tokens(req.content)`（K 决策）
- [ ]  末行决策矩阵「孤儿 human 改内容重发」discarded 路径同事务 `session.context_token_count -= estimate_tokens(orphan.content)`
- [ ]  commit② 不更新 `last_active_at`
- [ ]  `ChatStreamRequest.session_id` 字段 docstring 补「前端 hint；后端按日切策略强制重判，不一致时通过 SSE `session_meta.session_id` 回灌生效 sid」
- [ ]  SSE `session_meta` payload docstring 补「`session_id` 始终为服务端最终生效 sid」
- [ ]  删除 `_truncate_title` 函数与 `import re`
...
- [ ]  查 `ChildProfile` 取 `age` / `gender`
- [ ]  `history = await build_context(sid, db)`
- [ ]  构造 `initial_state["messages"] = [build_system_prompt(age, gender), *history, HumanMessage(content=req.content)]`
- [ ]  commit② ai 消息落库同事务：`session.context_token_count += estimate_tokens(ai_content)`
- [ ]  commit② 之后**内联** `if session.context_token_count >= CONTEXT_COMPRESS_THRESHOLD_TOKENS: log.warning("context exceeded threshold", extra={"session_id": str(sid), "token_count": session.context_token_count})`
- [ ]  以 `asyncio.create_task(enqueue_audit(sid))` 启动审查（不 await）；**不再** `create_task(compress_context_if_needed(...))`（已撤销）
- [ ]  立即 yield end 帧
- [ ]  `graph.py` `enqueue_audit` 仍保持 no-op stub（基线 §7.6 M8 替换）
...
- [ ]  `SessionListResponse` schema 加顶层字段 `today_session_id: UUID | None`（**取代** v1.0 草案中的 `current_logical_day` 字符串）
- [ ]  `/api/me/sessions` GET handler：
...
- [ ]  全套 `pytest` 跑绿（≥440 passed）
- [ ]  新建空白 M6-patch3 偏差档 Notion 子页（执行中按需登记）
- [ ]  在偏差档 / CHANGELOG 中跨里程碑备忘：
...
- [ ]  `git log --oneline` 检查 10 个 commit 全 Conventional Commits 小写前缀
