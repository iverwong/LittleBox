...
| Tool handler DB 异常 catch | (a) 宽 catch `{DBAPIError, ResourceClosedError}` / (b) 窄 catch `{InterfaceError, OperationalError}` | **(a) 宽 catch** | 真实故障 `statement_timeout` 抛基类 `DBAPIError`(probe 锁死),窄 catch 会漏 → child skip;`logger.exception` 留 stack trace 保留 bug 可见性 |
...
`_get_recent_reports` 之外(它只查历史),`run_daily_reports` 路径上构建 `ExpertContextSchema` 时填 `session_id`。

具体获取:从 `audit.RollingSummary.session_id` 反查当日唯一 session(已经在 `load_context` 里 `select(RollingSummary).where(...in_(owned_sids))` 拉到),取第一条的 `session_id` 填入。
...
_CATCHABLE_DB_ERRORS = (DBAPIError, ResourceClosedError)

def _with_db_error_handling(handler):
    """包装 tool handler:DB 错误转 error ToolMessage,非 DB 异常照旧上抛。

    Catch 列表依据见 backend/scripts/probe_sa_asyncpg_exceptions.py
    (2026-06-24 跑出的真实故障 → 异常类型映射)。

    Trade-off:ProgrammingError / IntegrityError 是 DBAPIError 子类,代码 bug
    也会被 catch。LLM 看到"数据库错误"但实际是 bug。logger.exception 留 stack
    trace 供开发定位,日志里 bug 完整可查。窄 catch({InterfaceError, OperationalError})
    会漏 statement_timeout(抛基类 DBAPIError,probe 锁死),导致 child skip 严重故障,
    所以选宽 catch。
    """
    @wraps(handler)
    async def wrapper(args, runtime, tool_call_id):
        ...
        except _CATCHABLE_DB_ERRORS as exc:
            logger.exception(
...
| 故障 | SA 顶层异常 | MRO |
|---|---|---|
| 杀 backend(其他 conn 杀) | `sqlalchemy.exc.InterfaceError` | `InterfaceError -> DBAPIError -> ...` |
| `statement_timeout` 触发 | `sqlalchemy.exc.DBAPIError`(**基类!**) | `DBAPIError -> ...` |
| `use_closed_connection` | `sqlalchemy.exc.ResourceClosedError` | `ResourceClosedError -> InvalidRequestError -> SQLAlchemyError -> ...` |
| SQL 语法错 | `sqlalchemy.exc.ProgrammingError` | `ProgrammingError -> DatabaseError -> DBAPIError -> ...` |
| 表不存在 | `sqlalchemy.exc.ProgrammingError` | 同上 |
| UNIQUE 冲突 | `sqlalchemy.exc.IntegrityError` | `IntegrityError -> DatabaseError -> DBAPIError -> ...` |

第一行 `InterfaceError` 与第三行 `ResourceClosedError` 不在同一棵树上(后者在 `SQLAlchemyError` 另一支),所以 catch 列表需要 `DBAPIError + ResourceClosedError` 两类。
...
"""
# 跑 7 个场景,逐个打印 outer type + MRO + root cause + message
# 详见 commit 时落地版本
```

### 5.2 `tests/expert/test_tool_error_handling.py`(新增)

**3 个 case,每个触发真实 DB 故障,不在测试里 `raise X`:**
...
- `test_search_history_use_closed_connection_returns_error_tool_message`:async with 退出后 conn 已 close,再 execute → 真实 `ResourceClosedError` → 装饰器兜住

每个 case 验证:
- 返回 `ToolMessage`,`tool_call_id` 透传
- payload 含自然语言 error 字段(中文"数据库"等关键字)
- `logger.exception` 触发,`caplog.records` 含 stack trace
...
| `tests/expert/test_tool_error_handling.py` | 新增 3 个真实 DB 故障 case |
| `tests/integration/test_smoke.py` | worker fixture 路径同步 |
...
| 项 | 风险 | 缓解 |
|---|---|---|
| `statement_timeout` 抛基类 `DBAPIError` | 宽 catch 会连 `ProgrammingError`(代码 bug)一起兜住 | `logger.exception` 留 stack trace 供开发定位;LLM 视角行为不变 |
